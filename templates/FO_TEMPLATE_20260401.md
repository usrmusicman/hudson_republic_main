# Form Name

---

## Definitions
List any ambiguous terms with definitions here.

---

## Preamble
This is a short summary of what is in the legislative instrument.

---

## Form Draft
This contains the contents of form.  
A form can be anything complementing legislative instruments in the ease of administration.  
Contents can include templates for: licenses, permits, notices, administrative forms and many other types of administrative instruments. 

---

## Disclaimer
**All variable types require lowercase names and all multi-word variable names require the use of underscores (_), instead of spaces ( ).**

---

## Reference Dependencies
List any applicable legislative instruments (in alphabetical order) that this one depends on using the FQLN.  
All legislative instruments are accepted as dependencies.

Dependencies
* **[CA_EXAMPLE4_20260401](path/to/legislative/instrument/CA_EXAMPLE4_20260401.md)**
* **[LA_EXAMPLE1_20260401](path/to/legislative/instrument/LA_EXAMPLE1_20260401.md)**
* **[LB_EXAMPLE2_20260401](path/to/legislative/instrument/LB_EXAMPLE2_20260401.md)**

---

**Original Author**:

**Executive Office Signature**: 

**FQLN**: FO_[NAME]_[YYYYMMDD]  
**Revision**: [REVISION_IN_8_HEXADECIMAL_DIGITS]
