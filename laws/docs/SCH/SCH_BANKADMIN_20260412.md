# Banking Administration Schedule

---

## Definitions

- **Bullion Beaver**: The base unit of account in the Hudson Republic’s sound money system. 32,768 Bullion Beavers (2¹⁵) equal 1 troy ounce of gold.  
- **FHI Audit Fee**: The annual compliance audit fee required to maintain Financial Holdings Insurance.  
- **MTI Counterfeit (Without Terminal) Penalty**: Penalty assessed for a Merchant Transaction Insurance breach on physical bullion transactions of 1 troy ounce of gold or more when no XRF verification terminal is used.  
- **MTI Counterfeit (With Terminal) Penalty**: Penalty assessed for the same breach when an XRF verification terminal is provided.  
- **Inspector Exam Certification Fee**: The non-refundable fee required for initial certification and renewal of FHI- and MTI-certified inspectors and examiners.  
- **Financial Institution**: Any bank, vault, or entity operating under Financial Holdings Insurance in the Hudson Republic.  
- **Number of Months Valid**: The duration (in months) for which the listed expense, fee, or penalty remains valid before it must be re-assessed, renewed, or expires.

---

## Preamble

This schedule sets out the standardized administrative expenses, fees, and penalties for Financial Holdings Insurance (FHI) and Merchant Transaction Insurance (MTI). These charges protect the integrity of the Beaver Ledger by ensuring all institutions and merchants handling physical bullion maintain verifiable, chemistry-backed compliance and deter fraud or counterfeiting. All amounts are expressed in Bullion Beavers and include a validity period for clarity and auditability.

---

## Usage

### Banking Administration Schedule

**Expense**: This is the name of the expense, fee, or penalty in question.  
**Financial Institution**: This is the name of the organization the expense applies to.  
**Riding Name**: This is the riding in which the expense is assessed.  
**Amount in Bullion Beavers**: This is the amount expressed in the smallest unit of Bullion Beavers.  
**Number of Months Valid**: This is the number of months for which the listed item remains valid before it must be re-assessed, renewed, or expires.

---

## Schedules

**Administration Expenses**

| Expense                                      | Financial Institution | Riding Name   | Amount in Bullion Beavers | Number of Months Valid |
|----------------------------------------------|-----------------------|---------------|---------------------------|------------------------|
| **FHI Audit Fee**                            | organization          | riding_name   | 327,680                   | 12                     |
| **MTI Counterfeit (Without Terminal) Penalty** | organization        | riding_name   | 32,768                    | 0                      |
| **MTI Counterfeit (With Terminal) Penalty**  | organization          | riding_name   | 512                       | 0                      |
| **Inspector Exam Certification Fee**         | organization          | riding_name   | 32,768                    | 60                     |

---

## Disclaimer

**All tags require lowercase names and all multi-word tags require the use of underscores (_), instead of spaces ( ).**

---

## Reference Dependencies

Dependencies (in alphabetical order)  
* **[CA_BANKANDRESERVE_20260401](../CA/CA_BANKANDRESERVE_20260401.md)**  
* **[CA_LEGALTENDER_20260401](../CA/CA_LEGALTENDER_20260401.md)**  

---

**Original Author**:  

**House Signature**:   

**Senate Signature**:   

**Executive Office Signature**:   

**FQLN**: SCH_BANKADMIN_20260412  
**Revision**: 00000002
