# Immigration Application Schedule

---

## Hard Dependencies
List any legislative instrument(s) (in alphabetical order) that this article must depend on. List the FQLN(s) below. Any FQLN(s) mentioned in other sections are considered to be references and not actual dependencies.  
Only **Constitutional Articles (CA)**, **Legislative Articles (LA)** and **Legislative Codes (CO)** can be listed here.

Dependencies
* **[LA_IMMIGRATION_20260401](../LA/LA_IMMIGRATION_20260401.md)**

---

## Definitions
- **Guest**: Temporary visitor status with limited stay rights.  
- **Resident**: Holder of an Authorized Resident Certificate (ARC).  
- **Naturalized Citizen**: Person granted full citizenship through the Pathway to Citizenship (PTC).  
- **Pathway to Citizenship (PTC)**: The formal multi-step process for Residents to become full citizens.  
- **Authorized Resident Certificate (ARC)**: 10-year renewable residency permit.  
- **FARM Visa**: Foreign Agricultural and Resource Management temporary work visa.  
- **HCB8**: The smallest unit of Hudson Copper Bullion (1/8 troy ounce of 95% pure copper), used as the base unit for all fees in this schedule.  
- **Per Count**: Per violation or per day of violation, as applicable.

---

## Preamble
This schedule sets out the standardized fees, conditions, renewal rules, and administrative requirements for all immigration programs in the Hudson Republic. It enforces a controlled, merit-based system that prioritizes national security, economic sustainability, cultural cohesion, and citizen sovereignty. All fees, fines, and penalties are expressed in HCB8 (Hudson Copper Bullion units) and must be paid exclusively in physical bullion. This schedule is purely referential and derives its full authority from the **[Immigration and Citizenship Act](../LA/LA_IMMIGRATION_20260401.md)**.

---

## Usage
Understanding the schedule(s) implementations provided.

### Immigration Program Schedule
This table governs all non-citizen entry and stay programs.  
- **Program Type** determines the maximum stay, fees, and conditions.  
- **Issuance Date** and **Expiry Date** are recorded at the time of approval.  
- Fees are non-refundable and payable in physical bullion only.

### Pathway to Citizenship (PTC) Schedule
This table governs the final stage for Residents seeking full citizenship.  
- All PTC steps must be completed while holding a valid ARC.  
- Fees are non-refundable and payable in physical bullion only.

---

## Schedules
This is where the schedules can be found for manual usage or as part of an automation.

### Immigration Program Table
**Table Schedule Type**

| Immigration Status | Program Type | Issuance Date (YYYYMMDD) | Expiry Date (YYYYMMDD) | Maximum Length of Stay (months) | Application Fee (HCB8) | Renewal Period (months) | Renewal Fee (HCB8) | Conditions of Stay | Fine for Non-Compliance (per count) (HCB8) | Ban Period (months) |
|--------------------|--------------|--------------------------|------------------------|---------------------------------|------------------------|-------------------------|--------------------|--------------------|--------------------------------------------|---------------------|
| guest              | guest        | YYYYMMDD                 | YYYYMMDD               | 24                              | 512                    | 3                       | 512                | short_description  | 512                                        | 60                  |
| resident           | farm         | YYYYMMDD                 | YYYYMMDD               | 6                               | 4,096                  | 12                      | 4,096              | short_description  | 4,096                                      | 60                  |
| resident           | arc          | YYYYMMDD                 | YYYYMMDD               | 120                             | 32,768                 | 0                       | 0                  | short_description  | 512                                        | 60                  |

### Pathway to Citizenship (PTC) Table
**Table Schedule Type**

| Name of Expense     | Fee (HCB8) | Date of Appointment (YYYYMMDD) | Date of Acknowledgement (YYYYMMDD) |
|---------------------|------------|--------------------------------|------------------------------------|
| Interview 1         | 32,768     | YYYYMMDD                       | YYYYMMDD                           |
| Interview 2         | 32,768     | YYYYMMDD                       | YYYYMMDD                           |
| Written Exam        | 32,768     | YYYYMMDD                       | YYYYMMDD                           |

**Notes**  
- “Per count” means per violation or per day of violation.  
- FARM overstay: the individual pays the standard guest overstay fine (512 HCB8 per day); the sponsoring company pays a one-time penalty of 4,096 HCB8 per count.  
- All dates use YYYYMMDD format. All fees are non-refundable and payable only in physical bullion.

---

## Disclaimer
**All tags require lowercase names and all multi-word tags require the use of underscores (_), instead of spaces ( ).**

---

**Original Author**:

**House Signature** (only if attached to a parent instrument):

**Senate Signature** (only if attached to a parent instrument):

**Executive Office Signature**:

**FQLN**: SCH_IMMIGRATIONAPP_20260413
